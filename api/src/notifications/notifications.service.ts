...
export class NotificationsService {
  ...
  async _sendNotification(
    ...
    try {
      ...
      this.logDelivery(ctx, 'notification_pipeline_failed', { err: e, mailType });
  ...
  async sendInAppOnly(
    ...
    try {
      const notifications = await this.repo.bulkCreateByRecipent(recipients, partial);
  ...
  async sendBankAccountApprovedBuyerInApp(recipientEmail: string): Promise<void> {
    const link = `${config.appDomain}/dashboard/account-details`;
  ...
  async sendPaymentInitiatedSupplier(
    ...
    const props: EmailRichProps = {
      ...
      details: [
        { label: 'Amount: ', value: `${opts.amount} ${opts.currency.toUpperCase()}` },
  ...
  async sendOsnDepositCompletedBuyerInApp(
    ...
    await this.sendInAppOnly(
      ...
      { eventName: 'osn_deposit_completed_buyer_in_app', entityId: opts.dealId },
    );
  }
  ...
  async sendOsnDepositCompletedAdminInApp(
    ...
    await this.sendInAppOnly(
      ...
      { eventName: 'osn_deposit_completed_admin_in_app', entityId: opts.dealId },
    );
  }
  ...
  async sendOsnDepositCompletedSupplierEmailAndInApp(
    ...
    await this.sendInAppOnly(
      ...
      { eventName: 'osn_deposit_completed_supplier_in_app', entityId: opts.dealId },
    );
    await this.sendRichEmailOnly(supplierEmail, props, {
  ...
  async sendSupplierDocumentsVerifiedEmailAndInApp(
    ...
    await this.sendInAppOnly(
      ...
      { eventName: 'supplier_documents_verified_in_app', entityId: opts.dealId },
    );
    await this.sendRichEmailOnly(supplierEmail, props, {
