...
export class AdminDashboardFeeQuoteService {
  ...
  async getFeeQuote(
    ...
    if (!token) {
      this.log.error(
        'Missing internal token (set ADMIN_DASHBOARD_INTERNAL_TOKEN)',
      );
      throw new InternalServerError(
        'ADMIN_DASHBOARD_INTERNAL_TOKEN is not configured',
      );
    }
    ...
    try {
      ...
      ) as AdminFeeQuoteApiResponse | null;
      ...
      if (!res.ok) {
        ...
        this.log.warn(
          `Admin fee quote HTTP error: ${msg} (status=${res.status}, url=${url})`,
        );
      ...
      if (!parsed) {
        ...
        throw new BadRequestError(
          'Invalid fee quote response from admin dashboard',
        );
      }
      ...
      if (parsed.success === false) {
        const msg =
          parsed.message ||
          parsed.error ||
          'Admin fee quote reported success: false';
      ...
      if (parsed.success !== true || !parsed.data) {
        const msg =
          parsed.message ||
          'Invalid fee quote response from admin dashboard (missing data)';
        this.log.warn(
          `Admin fee quote missing success/data — ${msg} (url=${url})`,
        );
        throw new BadRequestError(msg);
      }
      ...
      if (e?.name === 'AbortError') {
        throw new BadRequestError(
          `Fee quote request timed out after ${this.timeoutMs}ms`,
        );
      }
