...
export class AdminDashboardOsnRuntimeService {
  ...
  async fetchOsnRuntimeSettings(): Promise<OsnRuntimeSettingsResolved> {
    ...
    try {
      ...
      const parsed = (text ? safeJsonParse(text) : null) as AdminOsnRuntimeApiResponse | null;
      ...
      if (!parsed) {
        throw new BadRequestError('Invalid OSN runtime response from admin-dashboard (empty body)');
      }
      ...
      if (!parsed.data) {
        throw new BadRequestError('Invalid OSN runtime response from admin-dashboard (missing data)');
      }
      ...
      return {
        ...
        defaultCurrencyCode: (d.defaultCurrencyCode || 'USDC').trim().toUpperCase(),
        defaultChainId: d.defaultChainId,
      };
    } catch (err: unknown) {
      const e = err as { name?: string; message?: string; cause?: unknown };
      if (e?.name === 'AbortError') {
        throw new BadRequestError(`OSN runtime settings request timed out after ${this.timeoutMs}ms`);
      }
      if (err instanceof BadRequestError) throw err;
      const causeMsg =
        e?.cause != null ? (e.cause instanceof Error ? e.cause.message : String(e.cause)) : '';
      const detail = [e?.message, causeMsg].filter(Boolean).join(' — ') || 'OSN runtime settings request failed';
