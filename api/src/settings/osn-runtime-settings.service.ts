...
function envFallback(): OsnRuntimeSettingsResolved {
  return {
    ...
    defaultCurrencyCode: (process.env.OSN_DEFAULT_CURRENCY_CODE || 'USDC')
      .trim()
      .toUpperCase(),
...
export class OsnRuntimeSettingsService {
  ...
  private cache: { value: OsnRuntimeSettingsResolved; at: number } | null =
    null;
  ...
  async getOsnRuntimeSettings(): Promise<OsnRuntimeSettingsResolved> {
    ...
    if (useAdminDashboard) {
      const resolved =
        await this.adminDashboardOsnRuntime.fetchOsnRuntimeSettings();
      this.cache = { value: resolved, at: now };
      return resolved;
    }

    this.log.debug(
      'OSN runtime: using TruMarket local Mongo + env (admin-dashboard URL/token not set)',
    );
  ...
  async upsertOsnRuntimeSettings(
    ...
    const normalized: Partial<TruMarketSettings> = {
      ...
      defaultOrganizationBankId: input.defaultOrganizationBankId || undefined,
