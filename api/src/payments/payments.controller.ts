...
export class PaymentsController {
  ...
  @ApiOperation({ summary: 'Admin: verify supplier payment documents (requires OSN deposit completed)' })
  @ApiResponse({ status: 200, type: PaymentResponseDto })
  async verifyPaymentDocuments(
    @Param('dealId') dealId: string,
    @Param('paymentId') paymentId: string,
  ): Promise<PaymentResponseDto> {
    const payment = await this.paymentsService.verifyPaymentDocumentsAsAdmin(dealId, paymentId);
    return new PaymentResponseDto(payment as any);
  }

  @Post(':paymentId/request-withdrawal')
  @AuthenticatedRestricted()
  @ApiOperation({ summary: 'Supplier: request withdrawal / offramp for a completed payment' })
  @ApiResponse({ status: 200, type: PaymentResponseDto })
