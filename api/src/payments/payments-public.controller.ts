...
import { AdminDashboardFeeQuoteService, OsnFeeQuoteData } from '@/admin-dashboard/admin-dashboard-fee-quote.service';
...
export class PaymentsPublicController {
  ...
  @ApiOperation({ summary: 'Pre-mint OSN fee quote (proxied from admin-dashboard)' })
  @ApiResponse({ status: 200, description: 'Normalized `data` from admin fee-quote' })
  async getOsnFeeQuote(@Body() dto: OsnFeeQuoteRequestDto): Promise<OsnFeeQuoteData> {
    return this.adminDashboardFeeQuoteService.getFeeQuote(dto.amount, dto.currency.trim().toUpperCase(), {
  ...
  async getPayment(@Request() req, @Param('id') id: string): Promise<PaymentResponseDto> {
  ...
  async listTransfers(@Request() req, @Param('id') id: string): Promise<PaymentProviderTransfer[]> {
    const user: User = req.user;
  ...
  @AuthenticatedRestricted()
  @ApiOperation({ summary: 'Create and auto-approve OSN mint request, then persist provider transfer' })
  @ApiResponse({ status: 201, type: 'object' })
  async createMintRequest(@Request() req, @Param('id') id: string, @Body() body: any) {
    const user: User = req.user;
