...
describe('PaymentsService notifications', () => {
  const makeService = () => {
    ...
    return { service, payments, deals, providerTransfers, notifications, users, osnService };
  };

  it('payment initiated notifications are idempotent (second call skipped)', async () => {
    const { service, payments, deals, providerTransfers, notifications, users } = makeService();
    ...
    users.findById
      .mockResolvedValueOnce({ id: 'b1', email: 'buyer@example.com', company: { name: 'Buyer Co' } })
      .mockResolvedValueOnce({ id: 's1', email: 'sup@example.com', company: { name: 'Sup Co' } });
    providerTransfers.findById.mockResolvedValue({ providerPaymentId: 'osn_pay_1' });
    users.findByRole.mockResolvedValue([{ email: 'admin@example.com', role: RoleType.ADMIN }]);
  ...
  it('does not send admin payment-initiated email when admin is the same address as supplier', async () => {
    ...
    users.findById
      .mockResolvedValueOnce({ id: 'b1', email: 'buyer@example.com', company: { name: 'Buyer Co' } })
      .mockResolvedValueOnce({ id: 's1', email: 'sup@example.com', company: { name: 'Sup Co' } });
    users.findByRole.mockResolvedValue([{ email: 'sup@example.com', role: RoleType.ADMIN }]);
  ...
  it('OSN deposit completed side effects are skipped when all flags already set', async () => {
    ...
    expect(notifications.sendOsnDepositCompletedBuyerEmail).not.toHaveBeenCalled();
  ...
  it('syncOsnPaymentStatusesFromProvider does not duplicate deposit notifications', async () => {
    const { service, payments, deals, providerTransfers, notifications, users, osnService } =
      makeService();
    ...
    expect(osnService.getPaymentHistoryStatus).toHaveBeenCalledWith('osn_pay_1');
    expect(notifications.sendOsnDepositCompletedBuyerEmail).toHaveBeenCalledTimes(1);
  });
});
