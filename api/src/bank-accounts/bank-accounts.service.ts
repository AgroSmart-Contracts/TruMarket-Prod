...
export class BankAccountsService {
    ...
    if (!isSupplierRequest) {
      ...
      if (!runtimeSettings.organizationId) {
        throw new BadRequestError(
          'OSN organizationId is not configured in admin settings or env',
        );
      }
      ...
      const isDefault =
        status === BankAccountStatus.ACTIVE ? !existingDefault : false;
  ...
  async updateForUser(
    ...
    if (user.accountType !== 'supplier') {
      ...
      throw new BadRequestError(
        'Only supplier accounts can be updated in-place',
      );
    }
  ...
  async syncOsnOrganizationBanks(options?: {
    ...
    if (options?.pendingOnly)
      localFilter.status = BankAccountStatus.PENDING_APPROVAL;
    ...
    const ensureFallbackOrganizationId = async (): Promise<
      string | undefined
    > => {
      ...
      try {
        ...
        logger.warn(
          { err },
          'OSN bank sync: failed to resolve runtime fallback organization',
        );
    ...
    for (const local of localAccounts) {
      ...
      if (!orgId) {
        ...
        await this.bankAccounts.updateById(local.id, {
          lastSyncedAt: now,
          syncError:
            'Missing providerOrganizationId and fallback runtime organizationId',
          updatedAt: now,
        } as any);
        logger.warn(
          {
      ...
      for (const local of groupAccounts) {
        ...
        if (!remote) {
          ...
          const out = await this.bankAccounts.updateById(local.id, {
            lastSyncedAt: now,
            syncError:
              'OSN bank account id not found in organization-banks list',
            updatedAt: now,
          } as any);
          if (out) updated += 1;
          errors += 1;
            ...
            } as any,
            {
              $set: {
                ...
                providerOrganizationId:
                  local.providerOrganizationId || organizationId,
  ...
  async syncPendingBuyerAccountsForUserIfStale(user: User): Promise<void> {
    ...
    try {
      await this.syncOsnOrganizationBanks({
        userId: user.id,
        pendingOnly: true,
      });
    } catch {
  ...
  private async ensureBuyerBankAccountApprovalNotifications(
    ...
    if (!account.approvalInAppNotificationSentAt) {
      ...
      try {
        await this.notificationsService.sendBankAccountApprovedBuyerInApp(
          user.email,
        );
        ...
        logger.warn(
          { err, bankAccountId },
          'bank account approval: in-app notification failed',
        );
      }
    } else {
      logger.debug(
    ...
    if (!refreshed?.approvalEmailSentAt) {
      ...
      try {
        await this.notificationsService.sendBankAccountApprovedBuyerEmail(
          user.email,
          {
        ...
        logger.info(
          { bankAccountId },
          'bank account approval: email persisted',
        );
      } catch (err) {
        logger.warn(
          { err, bankAccountId },
          'bank account approval: email failed',
        );
      }
    } else {
      logger.debug(
        { bankAccountId },
        'bank account approval: email skipped — already sent',
      );
    }
  }
