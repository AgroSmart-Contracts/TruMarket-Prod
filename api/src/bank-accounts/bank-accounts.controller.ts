...
export class BankAccountsController {
  ...
  @ApiOperation({
    summary: 'List bank accounts for current user (archived=false)',
  })
  ...
  @ApiOperation({
    summary:
      'Create a new bank account request (creates PENDING_APPROVAL record)',
  })
  ...
  @AuthenticatedRestricted()
  @ApiOperation({
    summary: 'Update bank account details in-place (supplier only)',
  })
  @ApiResponse({ status: 200, type: BankAccountResponseDto })
  async update(
    @Request() req,
  ...
  @ApiOperation({
    summary:
      'Sync OSN organization bank activation state into local DB (admin/system use)',
  })
  async syncOsnOrganizationBanks(): Promise<{
    updated: number;
    checked: number;
    errors: number;
    transitions: number;
