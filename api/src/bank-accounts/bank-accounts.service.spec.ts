...
describe('BankAccountsService', () => {
  const makeService = () => {
    ...
    return { service, bankAccounts, osnService, osnRuntimeSettings, users, notificationsService };
  };

  it('syncOsnOrganizationBanks transitions pending buyer account to ACTIVE when OSN is active', async () => {
    const { service, bankAccounts, osnService, osnRuntimeSettings, users, notificationsService } = makeService();
    ...
    bankAccounts.find.mockResolvedValueOnce([
      {
        id: 'ba_1',
        userId: 'u1',
    ...
    const result = await service.syncOsnOrganizationBanks({ pendingOnly: true });
    ...
    expect(notificationsService.sendBankAccountApprovedBuyerInApp).toHaveBeenCalledTimes(1);
    expect(notificationsService.sendBankAccountApprovedBuyerEmail).toHaveBeenCalledTimes(1);
  });
  ...
  it('does not send duplicate approval notifications when transition update is no-op', async () => {
    ...
    expect(notificationsService.sendBankAccountApprovedBuyerInApp).not.toHaveBeenCalled();
    expect(notificationsService.sendBankAccountApprovedBuyerEmail).not.toHaveBeenCalled();
  });
  ...
  it('groups by providerOrganizationId and syncs each organization separately', async () => {
    ...
    osnService.listOrganizationBanks
      .mockResolvedValueOnce([{ id: 'osn_bank_1', organization_id: 'org_1', is_active: false }])
      .mockResolvedValueOnce([{ id: 'osn_bank_2', organization_id: 'org_2', is_active: false }]);
    bankAccounts.update.mockResolvedValue(0);
    ...
    expect(osnService.listOrganizationBanks).toHaveBeenNthCalledWith(1, { organizationId: 'org_1' });
    expect(osnService.listOrganizationBanks).toHaveBeenNthCalledWith(2, { organizationId: 'org_2' });
  });

  it('uses runtime fallback org only for rows missing providerOrganizationId', async () => {
    const { service, bankAccounts, osnService, osnRuntimeSettings } = makeService();
  ...
  it('does not block known-org rows when runtime fallback resolution fails', async () => {
    ...
    osnRuntimeSettings.getOsnRuntimeSettings.mockRejectedValue(new Error('runtime unavailable'));
    bankAccounts.find.mockResolvedValueOnce([
      {
        id: 'ba_known',
        userId: 'u1',
