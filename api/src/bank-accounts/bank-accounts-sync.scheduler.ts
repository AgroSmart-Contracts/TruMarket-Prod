...
@Injectable()
export class BankAccountsSyncScheduler
  implements OnModuleInit, OnModuleDestroy
{
  ...
  onModuleInit() {
    ...
    const cronExpr = process.env.BANK_ACCOUNTS_SYNC_CRON || '*/2 * * * *'; // default every 2 minutes
    ...
    this.job = schedule.scheduleJob(cronExpr, async () => {
      try {
        ...
        logger.debug({ result }, 'Bank account OSN sync scheduler completed');
      } catch (err) {
