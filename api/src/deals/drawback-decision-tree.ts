...
import type { DrawbackChronologyValidation } from './drawback-chronology-validation';
...
function slotOnFile(
  onFile: Set<DrawbackRequirementSlot>,
  slot: DrawbackRequirementSlot,
): boolean {
...
export function evaluateDrawbackDecisionTree(
  ...
  if (!hasDj) {
    findings.push(
      finding(
        ...
        'p1_no_sworn_declaration',
        'p1_imported_input',
        'No sworn declaration (DJ) of imported inputs is on file. Drawback is sustained by the declared material, not the main product alone.',
      ),
    );
  } else {
  ...
  if (!slotOnFile(onFileSlots, 'customsDeclarationDam')) {
    ...
    const codigo13Hint =
      /c[oó]digo\s*13|cod\.?\s*13|restituci[oó]n|drawback|rs\s*381|r\.?\s*s\.?\s*381/.test(
        damBlob,
      );
  ...
  if (chronology.rejected && chronology.errors.length > 0) {
    for (const err of chronology.errors) {
      findings.push(finding('fail', 'x1_chronology', 'cross', err));
    }
  }
  ...
  for (const slot of missingRequiredSlots) {
    if (DRAWBACK_REQUIRED_SLOTS.includes(slot)) {
      const already = findings.some(
        (f) =>
          f.severity === 'fail' &&
          f.message.toLowerCase().includes(slot.replace(/([A-Z])/g, ' $1')),
      );
      if (!already) {
        findings.push(
          finding(
