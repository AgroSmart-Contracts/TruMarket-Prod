...
describe('DealsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: PdfClassificationService,
          useValue: {
            isPdf: jest.fn().mockReturnValue(false),
            shouldClassify: jest.fn().mockReturnValue(false),
        ...
        },
        {
          ...
          useValue: {
            sendInviteToSignupNotification: jest
              .fn()
              .mockResolvedValue(undefined),
            sendDealCreatedNotification: jest.fn().mockResolvedValue(undefined),
            sendMilestoneApprovedNotification: jest
              .fn()
              .mockResolvedValue(undefined),
          },
        },
  ...
  describe('listDeals', () => {
    it('should return a list of deals', async () => {
      ...
      const result = await dealsService.findDealsByUser(user, query);

      expect(dealsRepository.findByUser).toHaveBeenCalledWith(
        user.id,
        user.email,
        query,
      );
