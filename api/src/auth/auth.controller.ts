...
export class AuthController {
  ...
  async updateBankDetails(
    ...
    logger.debug(
      { userId: user.id },
      'Deprecated: update bank details (legacy)',
    );
    ...
    try {
      ...
      const created = await this.bankAccountsService.createForUser(
        dbUser as any,
        {
      ...
      await this.bankAccountsService.activateAndSetDefault(
        dbUser as any,
        created.id,
      );

      const bankAccounts = await this.bankAccountsService.listForUser(
        dbUser as any,
      );
      ...
      logger.error(
        { error, userId: user.id, bankDetails: body.bankDetails },
  ...
  async login(@Body() loginDto: LoginDto): Promise<LoginResponseDto> {
    ...
    try {
      ...
      logger.error(
        { error, web3authToken: web3authToken?.substring(0, 20) + '...' },
  ...
  @ApiResponse({
    ...
    type: UserDetailsResponseDto,
    description:
      'Get user profile by email (for checking supplier bank details)',
  })
