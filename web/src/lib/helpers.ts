...
import {
  inferCountryFromTradeLocation,
  normalizeTradeRouteField,
} from "./trade-location-normalize";
...
export const normalizeShipmentAgreementDetailsData = (apiData: ShippingDetails, isBuyer: boolean) => {
  ...
  const originRoute = normalizeTradeRouteField(apiData.origin, apiData.portOfOrigin);
  const destinationRoute = normalizeTradeRouteField(
    apiData.destination,
    apiData.portOfDestination,
  );
  ...
  const normalizedData = {
    ...
    origin: originRoute.country || apiData.origin,
    portOfOriginCity: originRoute.port || apiData.portOfOrigin,
    destination: destinationRoute.country || apiData.destination,
    buyerConfirmed: apiData.buyerConfirmed,
    supplierConfirmed: apiData.supplierConfirmed,
    portOfDestinationCity: destinationRoute.port || apiData.portOfDestination,
    ...
    addresseeCountry: (() => {
      const raw = isBuyer ? apiData.supplierCompany.country : apiData.buyerCompany.country;
      return inferCountryFromTradeLocation(raw) ?? raw;
    })(),
    ...
    country: (() => {
      const raw = isBuyer ? apiData.buyerCompany.country : apiData.supplierCompany.country;
      return inferCountryFromTradeLocation(raw) ?? raw;
    })(),
...
/** Join route parts (port, country, etc.) with ", " — skips empty values (no leading comma). */
...
export const hasDocsWithLength = (
  milestones: { docs?: unknown[] }[],
  hasDocuments?: boolean,
) => {
  if (hasDocuments === true) return true;
  if (!milestones?.length) return false;
  return milestones.some((item) => (item.docs?.length ?? 0) > 0);
};
...
export type CreatableSelectOption = { label: string; value: string };

/** Coerces Redux form state (string or option[]) into CreatableInput values. */
export function normalizeCreatableSelectOptions(raw: unknown): CreatableSelectOption[] {
  if (!raw) return [];
    ...
    return raw
      .map((item) => {
        if (typeof item === "string") {
          ...
          return email ? { label: email, value: email } : null;
        }
        if (item && typeof item === "object") {
          const o = item as { label?: string; value?: string };
          const value = String(o.value ?? o.label ?? "").trim();
          if (!value) return null;
          return { label: String(o.label ?? value), value };
        }
        return null;
      })
      .filter((item): item is CreatableSelectOption => item !== null);
  }
  if (typeof raw === "string") {
    return commaSeparatedStringToArray(raw)
      .filter(Boolean)
