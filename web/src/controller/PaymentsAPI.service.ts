...
export class PaymentsService {
  ...
  static async uploadPaymentDocuments(
    ...
    dealId: string,
    files: File[],
    options?: { documentUploadMode?: 'payment' | 'drawback' | 'normal' },
  ): Promise<Payment> {
    const formData = new FormData();
    files.forEach((file) => formData.append('file', file));
    const mode = options?.documentUploadMode ?? 'payment';
    formData.append('documentUploadMode', mode);
  ...
  static async syncPaymentTradeDocuments(
    paymentId: string,
    dealId: string,
  ): Promise<Payment> {
    const response = await axiosInstance.post(
      `/deals/${dealId}/payments/${paymentId}/trade-documents/sync`,
    );
    return response.data;
  }
}
