...
const AgreementDetailsView: React.FC<AgreementDetailsViewProps> = () => {
  ...
  const handleSubmitChanges = async () => {
    ...
    const normalizedData: ICreateShipmentParams = {
      ...
      milestones,
      // hardcodedValues
      investmentAmount: comparativeData.investmentAmount ?? 0,
      contractId: 0,
      roi: 0,
      netBalance: 0,
      revenue: 0,
  ...
  const modalContentData = {
    ...
    [AgreementDetailsModalContentEnum.SHIPMENT_CREATED]: {
      ...
      primaryOptionAction: () =>
        openInNewTab(
          `${process.env.NEXT_PUBLIC_DEAL_CHAIN_EXPLORER || process.env.NEXT_PUBLIC_BLOCKCHAIN_EXPLORER}/token/${process.env.NEXT_PUBLIC_DEAL_NFT_CONTRACT_ADDRESS || process.env.NEXT_PUBLIC_NFT_CONTRACT_ADDRESS}`,
        ),
