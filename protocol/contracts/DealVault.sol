 ...
 * @author TruMarket
 *
 * @dev Per-deal ERC-4626 vault deployed by `DealsManager` on `mint`.
 *
 * ## v2.0 (Lagoon liquidity pool)
 * Live investor deposits go to the shared Lagoon pool, not into this vault. This contract
 * still holds per-deal USDC for borrower repayment, admin transfers, and read-only
 * funding display in the app.
 *
 * ## Active entry points (v2.0)
 * - `transferToBorrower` — admin vault transfer (via DealsManager `transferFromVault`).
 * - `donate` — borrower repayment (via DealsManager `donateToDeal`).
 * - `complete` — unlock vault after borrower repayment exceeds `maxDeposit`.
 * - `pause` / `blockDeposits` — set at deal mint; direct investor deposits are disabled.
 *
 * ## Legacy entry points (v1.x investor path)
 * - `deposit` / `mint` / `redeem` / `withdraw` — direct investor funding. Retained for
 *   compatibility and tests; not used for v2.0 investor flows.
 *
 * @notice ERC-4626 vault (`DLS`) with inflation-attack protection (`_decimalsOffset = 6`).
 *         Owner is always the deploying `DealsManager` instance.
 */
