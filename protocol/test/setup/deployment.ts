...
export async function deploy(
  ...
  const DealVaultFactory = await hre.ethers.getContractFactory('DealVaultFactory');
  const vaultFactory = await DealVaultFactory.connect(accounts.deployerAccount).deploy();
